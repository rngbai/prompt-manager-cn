...
export function mountSidepanelFooter({ active = null, root = document.body } = {}) {
  ...
  footer.innerHTML = `
    <div class="footer-icons">
      <a class="footer-icon-link${active === 'opd' ? ' footer-icon-link-active' : ''}" data-footer-page="opd" href="${prefix}opd-settings.html" title="开放提示词数据库">
      ...
      <a class="footer-icon-link" href="https://buymeacoffee.com/jonathanbertholet" target="_blank" rel="noopener" title="请作者喝杯咖啡">
      ...
      <a class="footer-icon-link" href="https://chromewebstore.google.com/detail/open-prompt-manager/gmhaghdbihgenofhnmdbglbkbplolain" target="_blank" rel="noopener" title="评价插件">
      ...
      <a class="footer-icon-link${active === 'settings' ? ' footer-icon-link-active' : ''}" data-footer-page="settings" href="${prefix}settings.html" title="设置">
      ...
      <a class="footer-cta-link" href="${prefix}permissions/permissions.html" title="开始使用" target="_blank" rel="noopener">
        开始使用
      </a>
    </div>
  `;

  // Personal-library mode keeps only local settings; remove external/community links.
  footer.querySelectorAll('.footer-icon-link').forEach((link) => {
    if (link.dataset.footerPage !== 'settings') link.remove();
  });
  footer.querySelector('.footer-cta-link')?.remove();
