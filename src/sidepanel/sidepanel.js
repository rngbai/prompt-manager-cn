...
function openComposerView({ title = '', content = '', uuid = '', tags = [] } = {}) {
  ...
  if (submitButton) {
    submitButton.textContent = uuid ? '更新' : '保存提示词';
  }
  if (cancelButton) {
    cancelButton.textContent = uuid ? '取消' : '返回';
  }
...
async function insertPromptIntoActiveTab(prompt) {
  ...
  if (!tab?.id || !tab.url || !/^https?:/i.test(tab.url)) {
    showSidepanelToast('请先打开聊天网站，再点击提示词插入。', { error: true });
    return false;
  }
  ...
  if (result?.error === 'no_permission' || result?.error === 'no_input') {
    ...
    if (pickerResult?.error === 'permission_denied') {
      showSidepanelToast('请允许插件访问此页面，以便选择输入框。', { error: true });
      return false;
    }
    if (!pickerResult?.ok && pickerResult?.error !== 'picker_already_active') {
      showSidepanelToast(pickerResult?.error || '无法在此页面启动输入框选择器。', { error: true });
      return false;
    }
...
function mapInsertError(code) {
  switch (code) {
    case 'no_permission':
      return '此页面尚未启用，请在 AI 助手区域授予权限后重试。';
    case 'no_input':
      return '此页面没有找到聊天输入框。';
    case 'no_active_tab':
      return '请先打开聊天网站，再点击提示词插入。';
    case 'prompt_not_found':
      return '找不到该提示词，请刷新侧栏后重试。';
    case 'inject_failed':
    case 'handler_missing':
      return '无法连接此页面，请刷新标签页后重试。';
    default:
      return '无法插入该提示词，请重试。';
  }
}
...
/** COMMENT: Personal-library mode copies a prompt instead of requiring AI-site permissions. */
async function copyPromptToClipboard(prompt) {
  try {
    await navigator.clipboard.writeText(prompt.content);
    showSidepanelToast('提示词已复制到剪贴板。');
    return true;
  } catch (_) {
    showSidepanelToast('复制失败，请检查浏览器剪贴板权限。', { error: true });
    return false;
  }
}
...
async function refreshCustomWebsiteButton(customWebsiteBtn) {
  ...
  if (!tab?.url || !/^https?:/i.test(tab.url)) {
    ...
    customWebsiteBtn.title = '打开一个网站页面，然后点击此处选择输入框';
    return;
  }
  ...
  if (pinned) {
      customWebsiteBtn.title = `重置 ${hostname} 上的输入框${stored?.label ? `（${stored.label}）` : ''}，点击重新选择`;
  } else if (learned) {
      customWebsiteBtn.title = `重置 ${hostname} 上自动检测的输入框，点击重新选择`;
  } else {
    customWebsiteBtn.title = `选择 ${hostname} 上的输入框`;
  }
}
...
async function handleCustomWebsiteAction(customWebsiteBtn) {
  ...
  if (currentlyPinned || currentlyLearned) {
    ...
    if (result?.error === 'permission_denied') {
      window.alert('请允许插件访问此页面，以便选择输入框。');
      return;
    }
    if (!result?.ok && result?.error !== 'picker_already_active') {
      window.alert(result?.error || '无法重置此页面的输入框检测。');
      return;
    }
    await refreshCustomWebsiteButton(customWebsiteBtn);
  ...
  if (!result?.ok && result?.error !== 'picker_already_active') {
    window.alert(result?.error || '无法在此页面启动输入框选择器。');
    return;
  }
...
async function initSidepanelContent() {
  ...
  // Personal-library mode never gates the prompt list on website permissions.
  cachedPermissionAllowed = true;
...
function mapPublishError(code) {
  ...
  switch (key) {
  case 'not_registered':
    return '请先在开放提示词数据库设置中设置发布名称，再进行分享。';
  case 'permission_denied':
    return '分享需要访问提示词目录，请授予权限后重试。';
  case 'publish_disabled':
    return '开放提示词数据库设置中的分享功能已关闭。';
  case 'prompt_not_found':
    return '该提示词已不在你的库中。';
  case 'catalog_lookup_failed':
    return '无法连接提示词目录，请检查网络后重试。';
  case 'check_failed':
  case 'register_failed':
    return '无法注册发布名称，请从开放提示词数据库设置中重试。';
  case 'turnstile_failed':
    return '分享验证失败，请稍后重试。';
  case 'unauthorized':
    return '缺少发布者身份，请在设置中关闭再重新开启分享后重试。';
  case 'rate_limited':
    return '分享次数过多，请稍等片刻后重试。';
  case 'validation_failed':
    return '该提示词无法发布，请检查标题和内容。';
  case 'forbidden':
    return '该目录标识属于其他发布者。';
  case 'invalid_json':
  case 'Failed to fetch':
    return '无法连接 openpromptdatabase.com，请授予目录访问权限后重试。';
  case 'no_response':
    return '插件后台没有响应，请重新加载插件后重试。';
  default:
    return key
      ? `无法分享该提示词（${key}），请重试。`
      : '无法分享该提示词，请重试。';
...
async function renderTagsFilterBar(prompts, enableTagsOverride) {
  ...
  bar.appendChild(makePill('全部', 'all', selected === 'all'));
