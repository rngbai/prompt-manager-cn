...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function launchProviderFromOnboarding(providerKey, providerInfo) {
    return new Promise((resolve) => {
        ...
        chrome.runtime.sendMessage(
          ...
          (response) => {
            ...
            if (response?.error === 'permission_denied') {
              alert(`${providerKey} 的访问权限被拒绝，请在 Chrome 弹窗中允许网站访问后继续。`);
            }
      ...
      chrome.permissions.request({ origins }, (granted) => {
        if (chrome.runtime.lastError) {
          alert(`无法申请 ${providerKey} 的访问权限：${chrome.runtime.lastError.message}`);
          resolve({ ok: false, error: chrome.runtime.lastError.message });
          return;
        }
        if (!granted) {
  ...
  chrome.storage.local.get(['aiProvidersMap'], function (result) {
    ...
    providerShortcutsContainer.innerHTML = '<p>未找到 AI 助手数据。</p>';
  });
