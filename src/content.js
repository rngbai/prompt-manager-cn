...
class PromptUIManager {
  ...
  static BANNER_CONFIG = {
    ...
    id: 'info-banner-v3',
    html: `<span>
      <strong>新增：</strong>在任意网站使用提示词管理器。觉得好用吗？</br>
      <a href="https://chromewebstore.google.com/detail/open-prompt-manager/gmhaghdbihgenofhnmdbglbkbplolain" target="_blank" rel="noopener noreferrer" style="color:#3674B5;text-decoration:underline;">欢迎留下评价</a>！
    </span>`
  };
  ...
  static createVariableInputForm({ inputBox, content, variables, onSubmit }) {
    ...
    const varValues = {};
    const submitBtn = createEl('button', { innerHTML: '插入', className: `opm-button opm-${getMode()}` });
    const backBtn = createEl('button', { innerHTML: '返回', className: `opm-button opm-${getMode()}` });
    ...
    variables.forEach(v => {
      ...
      const inputField = createEl('textarea', {
        attributes: { rows: '2', placeholder: `请输入${displayLabel}` },
  ...
  static async buildEditPromptForm(prompt) {
    ...
    const saveBtn = createEl('button', { innerHTML: '保存修改', className: `opm-button opm-${getMode()}` });
