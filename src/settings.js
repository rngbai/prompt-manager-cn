...
function initKeyboardShortcutRecorder() {
  ...
  recordBtn.addEventListener('click', () => {
    ...
    recording = true;
    recordBtn.textContent = '请按键……（Esc 取消）';
    recordBtn.classList.add('is-recording');
    display.textContent = '正在监听……';
...
async function renderTagManagement() {
  ...
  const render = () => {
    ...
    finalOrder.forEach((tag, idx) => {
      ...
      handle.setAttribute('aria-label', `调整标签 ${tag} 的顺序`);
      ...
      removeBtn.setAttribute('aria-label', `从所有提示词中移除标签 ${tag}`);
...
async function renderWebsitePermissions() {
  ...
  entries.forEach((entry) => {
    ...
    removeBtn.setAttribute('aria-label', `移除对 ${entry.label} 的访问权限`);
    ...
    removeBtn.addEventListener('click', async () => {
      const confirmed = confirm(`确定移除对 ${entry.label} 的访问权限吗？`);
      if (!confirmed) return;
...
async function deleteAllPrompts() {
  if (confirm('确定要删除全部提示词吗？此操作无法撤销。')) {
    await setPrompts([]);
    setImportExportStatus('已删除全部提示词。');
    renderTagManagement().catch(console.error);
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (exportButton) {
    exportButton.addEventListener('click', async () => {
      try {
        await exportPrompts();
        setImportExportStatus('导出已开始，请查看下载文件夹。');
      } catch (err) {
        console.error(err);
        setImportExportStatus('导出失败。', true);
      }
    });
  }
  ...
  if (importButton && importFile) {
    ...
    importFile.addEventListener('change', async (event) => {
      ...
      try {
        ...
        setImportExportStatus('导入成功，提示词已合并。');
        renderTagManagement().catch(console.error);
      } catch (err) {
        console.error(err);
        setImportExportStatus(err?.message || '导入失败，JSON 文件格式无效。', true);
      }
    });
  }
