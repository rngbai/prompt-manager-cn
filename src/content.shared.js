...
(function registerPromptManagerShared() {
  function initPromptManagerShared() {
    ...
    const ICON_SVGS = {
      ...
      add: `<img src="${chrome.runtime.getURL('icons/new.svg')}" width="16" height="16" alt="新建提示词" title="新建提示词" style="filter: ${iconFilter()}">`,
      ...
      changelog: `<img src="${chrome.runtime.getURL('icons/notes.svg')}" width="16" height="16" alt="更新记录" title="更新记录" style="filter: ${iconFilter()}">`,
    };
    ...
    const TagUI = (() => {
      ...
      const createTagInput = ({ initialTags = [] } = {}) => {
        ...
        const input = createEl('input', { attributes: { type: 'text', placeholder: '输入标签后按回车' }, className: `opm-tag-input opm-${getMode()}` });
    ...
    const PromptUI = (() => {
      ...
      const Elements = {
        ...
        createEditablePromptItem(prompt, idx, reorder) {
          ...
          const deleteIcon = Elements.createIconButton('delete', (e) => {
            e.stopPropagation();
            if (confirm(`确定删除“${prompt.title}”吗？`)) window.PromptUIManager.deletePrompt(prompt.uuid);
          });
        ...
        createBottomMenu() {
          ...
          const search = createEl('input', {
            ...
            attributes: { type: 'text', placeholder: '搜索提示词', style: 'border-radius: 4px;' }
          });
        ...
        createLauncherModePicker({ getValue, onChange }) {
          ...
          const modes = [
            { value: 'standard', label: '悬浮按钮' },
            { value: 'hotCorner', label: '热区' },
            { value: 'invisible', label: '侧栏或快捷键' },
          ];
      ...
      const Views = {
        createPromptForm({ initialTitle = '', initialContent = '', submitLabel = '保存', onSubmit }) {
          ...
          const contentArea = createEl('textarea', {
            attributes: { placeholder: '输入提示词，可使用 #变量名# 插入动态内容' },
          ...
          saveBtn.addEventListener('click', async e => {
            ...
            if (!t || !c) { alert('请填写提示词标题和内容。'); return; }
        ...
        async createPromptCreationForm(prefill = '') {
          ...
          const saveBtn = createEl('button', { innerHTML: '新建提示词', className: `opm-button opm-${getMode()}` });
          saveBtn.addEventListener('click', async e => {
            e.stopPropagation();
            const t = titleIn.value.trim(), c = contentArea.value.trim();
            if (!t || !c) { alert('请填写提示词标题和内容。'); return; }
            ...
            if (!res.success) { alert('保存提示词失败。'); return; }
        ...
        createSettingsForm() {
          ...
          const shortcutTitle = createEl('div', { styles: { fontWeight: 'bold', fontSize: '14px', marginTop: '2px' }, innerHTML: '打开/关闭快捷键' });
          ...
          const recordShortcutBtn = createEl('button', { innerHTML: '录制', className: `opm-button opm-${getMode()}` });
          ...
          recordShortcutBtn.addEventListener('click', (e) => {
            ...
            if (shortcutRecordingHandler) {
              stopShortcutRecording();
              recordShortcutBtn.innerHTML = '录制';
              refreshShortcutDisplay().catch(() => {});
              return;
            }
            recordShortcutBtn.innerHTML = '请按键……';
            shortcutDisplay.innerHTML = '正在监听……';
          ...
          deleteAllBtn.addEventListener('click', async e => {
            e.stopPropagation();
            if (!confirm('确定要删除全部提示词吗？此操作无法撤销。')) return;
            try {
              ...
              const render = () => {
                ...
                finalOrder.forEach((tag, idx) => {
                  ...
                  removeBtn.addEventListener('click', async (e) => {
                    e.stopPropagation();
                    if (!confirm(`确定从所有提示词中移除标签“${tag}”吗？`)) return;
                    try {
          ...
          const communityTitle = createEl('div', { styles: { fontWeight: 'bold', fontSize: '13px', marginTop: '10px', opacity: 0.85 }, innerHTML: '支持与链接' });
        ...
        async createEditView() {
          ...
          prompts.forEach((p, idx) => {
            ...
            const deleteIcon = Elements.createIconButton('delete', () => { if (confirm(`确定删除“${p.title}”吗？`)) window.PromptUIManager.deletePrompt(p.uuid); });
            actions.append(editIcon, deleteIcon);
            item.append(dragHandle, info, actions);
